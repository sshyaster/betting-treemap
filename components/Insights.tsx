...
export default function Insights({ polyMarkets, kalshiMarkets, timeframe }: InsightsProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Polymarket Markets" value={polyMarkets.length.toString()} sub="Active events" />
        <StatCard label="Polymarket Volume" value={formatVolume(polyTotal)} sub={`${timeframe} total`} accent />
        <StatCard label="Kalshi Markets" value={kalshiMarkets.length.toString()} sub="Active events" />
        <StatCard label="Kalshi Volume" value={formatVolume(kalshiTotal)} sub={`${timeframe} total`} accent />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <DonutChart categories={polyCategories} total={polyTotal} />
          <div className="mt-4 space-y-2">
            {polyCategories.map(cat => (
              <CategoryBar key={cat.name} name={cat.name} volume={cat.volume} total={polyTotal} />
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <DonutChart categories={kalshiCategories} total={kalshiTotal} />
          <div className="mt-4 space-y-2">
            {kalshiCategories.map(cat => (
              <CategoryBar key={cat.name} name={cat.name} volume={cat.volume} total={kalshiTotal} />
            ))}
          </div>
        </div>
      </div>
          ...
          <div className="space-y-3">
            {topPoly.map((market, i) => (
              <HotMarketRow key={market.id} rank={i + 1} market={market} timeframe={timeframe}
                maxVol={getVolumeForTimeframe(topPoly[0], timeframe)} />
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="space-y-3">
            {topKalshi.length > 0 ? topKalshi.map((market, i) => (
              <HotMarketRow key={market.id} rank={i + 1} market={market} timeframe={timeframe}
                maxVol={getVolumeForTimeframe(topKalshi[0], timeframe)} />
            )) : (
...
function StatCard({ label, value, sub, accent }: { label: string; value: string; sub: string; accent?: boolean }) {
  return (
    <div className={`rounded-xl border p-5 ${accent ? 'bg-gray-900 text-white border-gray-900' : 'bg-white border-gray-200'}`}>
      <div className={`text-xs uppercase tracking-wider mb-2 ${accent ? 'text-gray-400' : 'text-gray-500'}`}>{label}</div>
...
function DonutChart({ categories, total }: { categories: { name: string; volume: number }[]; total: number }) {
  ...
  return (
    <div className="flex justify-center">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        <text x={center} y={center - 4} textAnchor="middle" fill="#111" fontSize="14" fontWeight="700">
          {formatVolume(total)}
        </text>
...
function CategoryBar({ name, volume, total }: { name: string; volume: number; total: number }) {
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-0.5">
          ...
          <span className="text-xs text-gray-500 ml-2">{formatVolume(volume)}</span>
        </div>
...
function HotMarketRow({ rank, market, timeframe, maxVol }: { rank: number; market: Market; timeframe: Timeframe; maxVol: number }) {
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <span className="text-xs font-bold text-gray-300 w-5 pt-0.5 text-right">{rank}</span>
        <div className="flex-1 min-w-0">
          <div className="text-sm text-gray-900 font-medium group-hover:text-blue-600 transition truncate">
            {market.title}
          </div>
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-xs font-semibold text-gray-600">{formatVolume(vol)}</span>
          </div>
